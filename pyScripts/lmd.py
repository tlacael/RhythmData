...
from linSVM_clf import *
from alt_gatherScalars import *
...
_DIM = [60,60]
#DEFAULT_HDFILE = "/Users/Tlacael/NYU/RhythmData/bigDim.hdf"
LMD_SCALAR = ""#"/Users/Tlacael/NYU/RhythmData/lmd_scalars.pkl"
    ...
    for f in collect_mat_files(dpath,dname, didx):
        ...
        if not PASS:
            ...
            dset.add_data(X=[x],y=[genres[g]],
                    metadata=[{'filename':os.path.split(f)[-1],'title':str(data['name'][0])}])
...
class RhythmDataset(Dataset):
    ...
    def __init__(self, lmdScalar=LMD_SCALAR,hdfile=DEFAULT_HDFILE, valid=None, test=None, dim=_DIM):
        Dataset.__init__(self, hdfile, valid=valid, test=test)
        self.scalars = []
        self.dim = dim
        if os.path.exists(lmdScalar):
            mu,sd = cPickle.load(open(lmdScalar,'r'))
            self.scalars += [mu,sd]
            print "scalar file "+ lmdScalar +" loaded"
    ...
    def get(self, ikey):
        x,y,z = Dataset.get(self, ikey)
        dim = self.dim
        if len(self.scalars):
            x = (x - self.scalars[0].reshape(1,dim[1],dim[0]))/self.scalars[1].reshape(1,dim[1],dim[0])
        return x, y, z
...
origDimX = 60;
origDimY = 60;
arch_lmd = [AffineArgs(weight_shape=(60*60,256)),
